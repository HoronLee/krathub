...
- [x] **CORS 中间件简化** - 删除 `app/micro-forge/service/internal/server/middleware/cors.go` 封装层，让 `pkg/middleware/cors` 的 `Middleware()` 方法直接接受 `*conf.CORS` (来自 `api/gen/go/conf/v1/conf.pb.go`)，所有空字段检测、默认值填充等操作在 pkg 内部完成，`http.go` 可直接调用 `cors.Middleware(c.Http.Cors)`
- [x] **PostgreSQL init.sql 外置** - 优化 `app/micro-forge/service/deployment/kubernetes/postgres.yaml`，将 `init.sql: |` 内联字段外置为独立文件（使用 ConfigMap 挂载或 kustomize configMapGenerator）
...
- [ ] **Docker 配置存在明文凭据** - `app/micro-forge/service/configs/config-docker.yaml` 中 DB/Redis/JWT 明文
