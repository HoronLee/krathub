...
此目录包含 Krathub 项目中所有服务的 Protobuf 定义文件。我们使用 [Buf](https://buf.build/) 作为 Protobuf 构建工具链。
...
| `krathub/` | 主服务入口，主要包含 HTTP API 定义 | `i_*.proto` (HTTP) |
...
### 1. 协议分类与命名
- **HTTP API (BFF/Gateway)**: 文件名必须以 `i_` 开头（如 `i_auth.proto`），生成 HTTP 路由代码。统一使用包名 `krathub.service.v1`。
