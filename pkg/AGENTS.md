...
`pkg/` 是 Krathub 项目的共享库层，提供跨服务的通用基础设施组件和工具函数。所有 `app/` 下的微服务都可以通过 `github.com/horonlee/krathub/pkg/{module}` 导入这些模块。
...
- `github.com/horonlee/krathub/api/gen/go/conf/v1` (配置 Proto)
...
import (
    ...
    conf "github.com/horonlee/krathub/api/gen/go/conf/v1"  // 3. 项目内
)
```
...
### 项目内部依赖
所有 pkg 模块可能依赖：
- `github.com/horonlee/krathub/api/gen/go/conf/v1` - 配置 Protobuf 定义（生成的代码）
