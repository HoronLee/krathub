...
`app/` 是 Krathub 项目的微服务实现层，包含所有微服务的业务逻辑和运行时代码。每个微服务都采用 DDD（领域驱动设计）分层架构，通过 Wire 进行依赖注入，遵循清晰的职责分离原则。
...
Krathub 采用经典的 DDD 分层架构，各层职责明确，依赖方向单向。
...
**场景**：在 `krathub` 服务中添加新模块（如 `product`）
...
option go_package = "github.com/horonlee/krathub/api/gen/go/product/service/v1;v1";
...
func main() {
    ...
    // 连接数据库
    db, _ := gorm.Open(mysql.Open("root:password@tcp(localhost:3306)/krathub"))
    g.UseDB(db)
...
# 查看状态
kubectl get pods -l app=krathub
kubectl logs -f deployment/krathub
```
...
# 3. 修改 Wire 配置（如有必要）
vim app/krathub/service/cmd/server/wire.go
...
```bash
# 查看生成的 Wire 代码
cat app/krathub/service/cmd/server/wire_gen.go

# 检查 Wire 依赖图
cd app/krathub/service/cmd/server
wire show
...
# 查看 OpenAPI 文档
cat app/krathub/service/openapi.yaml
```
...
```go
// 使用 Kratos 错误类型
import userv1 "github.com/horonlee/krathub/api/gen/go/user/service/v1"
