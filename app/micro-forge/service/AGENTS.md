# AGENTS.md - app/micro-forge/service/ 主服务实现
...
`app/micro-forge/service/` 是 micro-forge 项目的主微服务实现，采用模块化单体架构（Modular Monolith），在一个服务进程内组织多个业务域（auth、user、test）。这是一个生产级的微服务实现，展示了完整的 DDD 分层架构、Wire 依赖注入、前后端分离等最佳实践。
...
micro-forge 采用经典的 DDD（领域驱动设计）三层架构，层间依赖单向、职责清晰。
...
import (
    ...
    authv1 "github.com/horonlee/micro-forge/api/gen/go/auth/service/v1"
...
import (
    ...
    "github.com/horonlee/micro-forge/pkg/hash"
...
import (
    ...
    dao "github.com/horonlee/micro-forge/app/micro-forge/service/internal/data/gorm/dao"
...
// internal/biz/auth.go
import "github.com/horonlee/micro-forge/app/micro-forge/service/internal/data"
type AuthUsecase struct {
...
```bash
cd /Users/horonlee/projects/go/micro-forge/web
...
```bash
# 创建 proto 文件
mkdir -p /Users/horonlee/projects/go/micro-forge/api/protos/product/service/v1
...
```bash
# 1. 修改 proto 文件
vim /Users/horonlee/projects/go/micro-forge/api/protos/auth/service/v1/auth.proto
...
```bash
# 查看生成的 Wire 代码
cat /Users/horonlee/projects/go/micro-forge/app/micro-forge/service/cmd/server/wire_gen.go

# 检查 Wire 依赖图
cd /Users/horonlee/projects/go/micro-forge/app/micro-forge/service/cmd/server
wire show
...
# 查看 OpenAPI 文档
cat /Users/horonlee/projects/go/micro-forge/app/micro-forge/service/openapi.yaml

# 查看日志
tail -f /Users/horonlee/projects/go/micro-forge/app/micro-forge/service/logs/micro-forge.service.log
```
...
**上游依赖**（本目录依赖的其他目录）：
- `/Users/horonlee/projects/go/micro-forge/api/gen/go/` - 生成的 protobuf Go 代码
- `/Users/horonlee/projects/go/micro-forge/pkg/` - 共享库（jwt, redis, logger 等）
- `/Users/horonlee/projects/go/micro-forge/api/protos/conf/v1/` - 配置文件定义

**下游依赖**（依赖本目录的其他目录）：
- `/Users/horonlee/projects/go/micro-forge/deployment/` - 部署配置
