...
import (
	...
	"github.com/horonlee/micro-forge/app/micro-forge/service/internal/data/ent"
...
func NewData(entClient *ent.Client, c *conf.Data, l logger.Logger, client client.Client, redisClient *redis.Client) (*Data, func(), error) {
	...
	return &Data{
		entClient: entClient,
		log:       logger.NewHelper(l, logger.WithModule("data/data/micro-forge-service")),
		client:    client,
		redis:     redisClient,
	}, cleanup, nil
}
...
func NewEntClient(db *gorm.DB, cfg *conf.Data, app *conf.App, l logger.Logger) (*ent.Client, error) {
	...
	opts := []ent.Option{
		...
		ent.Log(logger.EntLogFuncFrom(l, "ent/data/micro-forge-service")),
	}
...
func NewDB(cfg *conf.Data, l logger.Logger) (*gorm.DB, error) {
	gormLogger := logger.GormLoggerFrom(l, "gorm/data/micro-forge-service")
	dbLog := logger.NewHelper(l,
		logger.WithModule("data/db/micro-forge-service"),
...
func NewRedis(cfg *conf.Data, l logger.Logger) (*redis.Client, func(), error) {
	...
	return redis.NewClient(redisConfig, logger.With(l, logger.WithModule("redis/data/micro-forge-service")))
}
