...
func NewHTTPMiddleware(
	...
	publicWhitelist := mwpkg.NewWhiteList(mwpkg.Exact,
		microforgev1.OperationAuthServiceLoginByEmailPassword,
		microforgev1.OperationAuthServiceRefreshToken,
		microforgev1.OperationAuthServiceSignupByEmail,
		microforgev1.OperationTestServiceTest,
		microforgev1.OperationTestServiceHello,
	)

	// User 级接口白名单（需要 User 权限但跳过 Admin 检查）
	userWhitelist := mwpkg.NewWhiteList(mwpkg.Exact,
		microforgev1.OperationUserServiceCurrentUserInfo,
		microforgev1.OperationUserServiceUpdateUser,
		microforgev1.OperationAuthServiceLogout,
		microforgev1.OperationTestServicePrivateTest,
	)
...
func NewHTTPServer(
	...
	microforgev1.RegisterUserServiceHTTPServer(srv, user)
	microforgev1.RegisterTestServiceHTTPServer(srv, test)
