...
`app/krathub/service/` 是 Krathub 项目的主微服务实现，采用模块化单体架构（Modular Monolith），在一个服务进程内组织多个业务域（auth、user、test）。这是一个生产级的微服务实现，展示了完整的 DDD 分层架构、Wire 依赖注入、前后端分离等最佳实践。
...
Krathub 采用经典的 DDD（领域驱动设计）三层架构，层间依赖单向、职责清晰。
...
// internal/biz/auth.go
import "github.com/horonlee/krathub/app/krathub/service/internal/data"
type AuthUsecase struct {
