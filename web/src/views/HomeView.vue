...
const handleSignup = async () => {
  if (!signupForm.name || !signupForm.email || !signupForm.password || !signupForm.passwordConfirm) {
  ...
  loading.value = true
  try {
    await api.signup(signupForm.name, signupForm.email, signupForm.password, signupForm.passwordConfirm)
    ...
    <div class="max-w-md mx-auto">
        ...
        <el-tabs v-model="activeTab">
          ...
          <el-tab-pane label="注册" name="signup">
            <el-form @submit.prevent="handleSignup">
              ...
              <el-form-item label="密码">
                <el-input v-model="signupForm.password" type="password" placeholder="5-10个字符" />
              </el-form-item>
              <el-form-item label="确认密码">
                <el-input v-model="signupForm.passwordConfirm" type="password" placeholder="再次输入密码" />
              </el-form-item>
              <el-button type="primary" @click="handleSignup" :loading="loading" class="w-full">
                注册
              </el-button>
            </el-form>
          </el-tab-pane>
      ...
      <div v-else class="bg-white rounded-lg shadow p-6">
        ...
        <div class="space-y-2">
          ...
          <p><span class="font-medium">用户名:</span> {{ authStore.user?.name }}</p>
          <p><span class="font-medium">角色:</span> {{ authStore.user?.role }}</p>
        </div>
        <el-button @click="handleLogout" class="w-full mt-4">退出登录</el-button>
      </div>
    </div>
  </div>
</template>